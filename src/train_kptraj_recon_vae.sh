...
model_configs=(
    ...
    "traj_recon_vae_kl_small" 
    "traj_recon_vae_kl_annealing"
)
    ...
    if [[ $model_config == *"_vae"* ]]; then 
        ...
        do 
            ...
            if [ $log = 'save' ]; then 
                ...
                python3 train_kptraj_recon_vae.py --dataset_dir $traj_recon_dataset --training_tag $training_tag --config "../config/${model_config}.yaml" > $output_log
                python3 plot_history.py $output_log

            else 

                python3 train_kptraj_recon_vae.py --dataset_dir $traj_recon_dataset --training_tag $training_tag --config "../config/${model_config}.yaml"
