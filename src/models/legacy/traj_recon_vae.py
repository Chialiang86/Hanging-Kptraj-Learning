...
class  AllEncoder(nn.Module):
    ...
    def forward(self, traj_feats):
        ...
        z = F.leaky_relu(self.mlp3(z))
        mu = self.get_mu(z)
        logvar = self.get_logvar(z)
...
class AllDecoder(nn.Module):
    def __init__(self, z_feat_dim=64, hidden_dim=128, num_steps=30, wpt_dim=6):
...
class TrajRecon(nn.Module):
    def __init__(self, hidden_dim=128, z_feat_dim=64, num_steps=30, wpt_dim=6, 
        ...
        self.all_encoder = AllEncoder(hidden_dim=hidden_dim, z_feat_dim=z_feat_dim, num_steps=num_steps, wpt_dim=wpt_dim) # CVAE encoder
    ...
    def forward(self, traj):
        ...
        z_all, mu, logvar = self.all_encoder(traj)
    ...
    def get_loss(self, traj, lbd_kl=1.0):
    ...
    def get_loss_test_rotation(self, traj, lbd_kl=1.0):
        batch_size = traj.shape[0]
        recon_traj, mu, logvar = self.forward(traj)
