...
class TrajReconAffordanceNoFP4D(nn.Module):
    def __init__(self, pcd_feat_dim=256, traj_feat_dim=128,  
        ...
        super(TrajReconAffordanceNoFP4D, self).__init__()
    ...
    def forward(self, pcs, traj, contact_point):
        ...
        norm = torch.norm(pcs - contact_point.unsqueeze(1), dim=2).unsqueeze(2)
        pcs_copy = pcs_copy.repeat(1, 1, 2)
        pcs_copy = torch.cat((pcs_copy, norm), dim=2)
    ...
    def sample(self, pcs, contact_point):
        ...
        norm = torch.norm(pcs - contact_point.unsqueeze(1), dim=2).unsqueeze(2)
        pcs_copy = pcs_copy.repeat(1, 1, 2)
        pcs_copy = torch.cat((pcs_copy, norm), dim=2) # 7d point cloud
