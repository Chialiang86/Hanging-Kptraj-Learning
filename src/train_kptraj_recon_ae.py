...
def train(args):
    ...
    for epoch in range(start_epoch, stop_epoch + 1):
        ...
        # training
        train_total_losses = []
        for i_batch, sample_trajs in tqdm(enumerate(train_loader)):
            ...
            # forward pass
            losses = network.get_loss(sample_trajs)  # B x 2, B x F x N
        ...
        # validation
        val_total_losses = []
        for i_batch, sample_trajs in tqdm(enumerate(val_loader)):
...
def test(args):
    ...
    print(f'[INFO : model path : {weight_path}]')
    print(f'[INFO : dataset mode : {dataset_mode}]')
...
if __name__=="__main__":
    ...
    parser.add_argument('--checkpoint_dir', '-cd', type=str, default='checkpoints', help="'training_mode=train' only")
    
    # other info
    parser.add_argument('--device', '-dv', type=str, default="cuda")
    parser.add_argument('--config', '-cfg', type=str, default='../config/traj_recon_ae.yaml')
