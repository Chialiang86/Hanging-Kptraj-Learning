...
def test(args):
    ...
    for inference_hook_path in inference_hook_paths:
        ...
        #              3 # devil
        # category = int(hook_name.split('_')[-1])
        ...
        if hook_name in template_hook_names:
            ...
            # easy_cnt   += 1 if 'easy'   in difficulty_dict[hook_name] == 'easy' else 0
            # normal_cnt += 1 if 'normal' in difficulty_dict[hook_name] == 'normal' else 0
            # hard_cnt   += 1 if 'hard'   in difficulty_dict[hook_name] == 'hard' else 0
            # devil_cnt  += 1 if 'devil'  in difficulty_dict[hook_name] == 'devil' else 0
    ...
    for sid, pcd in enumerate(tqdm(hook_pcds)):
        ...
        # category = int(hook_name.split('_')[-1])
        # difficulty = difficulty_dict[hook_name]
        ...
        target_category, contact_point, affordance, recon_trajs = network.sample(points_batch, 
                                                                                    template_trajs, 
                                                                                    # difficulty=gt_category if use_gt_cls else None, 
                                                                                    difficulty=None, 
                                                                                    use_gt_cp=use_gt_cp, 
                                                                                    use_temp=use_temp,
                                                                                    return_feat=False)
        ...
        # cls_acc += torch.mean((target_difficulty == gt_category).float())
